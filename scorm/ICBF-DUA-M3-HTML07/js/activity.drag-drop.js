...
function setValues() {
  ...
  for (i = 1; i <= totalDrops; i++) {
    ...
    var drop = "<div class='frame-drop'><div class='frame-inner-drop row'><div id='drop_" + i + "' class ='droppable'></div></div></div>";

    if (i <= totalDrops/2) {
      $("#drop-container1").append(drop);
    }
    else {
    ...
    var dropId = "#drop_" + i;
    var dropCont = "<div class='drop-content'>" + drops[i].content + "</div>";
  ...
  for (i = 1; i <= totalDrags; i++) {
    ...
    var drag = "<div id='drag_" + i + "' class='col-lg-3 col-md-6'></>";
